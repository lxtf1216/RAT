...
class FSDPTrainer:
    ...
    def __init__(self, config):
        ...
        self.print_info()
        if self.gpu_id != -1:
            self.task_wrapper = FSDP(self.task,
                                     auto_wrap_policy=self.task.get_fsdp_plicy(),
                                     sharding_strategy=ShardingStrategy(self.config.trainer.fsdp.sharding_strategy),
                                     device_id=torch.cuda.current_device(),
                                     use_orig_params=True,
            )
            if self.config.trainer.torch_compile:
                self.task_wrapper = torch.compile(self.task_wrapper)
                # for layer_id, transformer_block in self.task_wrapper.module.backbone.layers.named_children():
                #     torch._dynamo.reset()
                #     transformer_block = torch.compile(transformer_block)
                #     self.task_wrapper.module.backbone.layers.register_module(layer_id, transformer_block)
            print("FSDP module is {}".format(self.task_wrapper))
    ...
    def build_optimizers(self, ):
        self.optimizer = util_config.instantiate(optimizer_registry, self.config.optim.optimizer, self.task_wrapper.parameters())
    ...
    def print_info(self, type="init"):
        ...
        if type == "init":
            ...
            print("the length of train loader is {}".format(len(self.train_loader)))
            print("the length of val loader is {}".format(len(self.val_loader)))
    ...
    def save_checkpoint(self, **resume_kwargs):
        ...
        if (self.step + 1) % self.save_interval == 0 or self.step + 1 == self.max_step:
            ...
            fullstate_config = FullStateDictConfig(rank0_only=True)
            fulloptim_config = FullOptimStateDictConfig(rank0_only=True)
            with FSDP.state_dict_type(self.task_wrapper, state_dict_type=StateDictType.FULL_STATE_DICT,
                                      state_dict_config=fullstate_config,
                                      optim_state_dict_config=fulloptim_config):
                model_state = self.task_wrapper.state_dict()
                optim_state = FSDP.optim_state_dict(self.task_wrapper, self.optimizer)
                if self.gpu_id == 0:
                    ckpt = {
                        "task": model_state,
                        "config": self.config,
    ...
    def load_checkpoint(self):
        ...
        if latest_checkpoint is not None:
            ...
            FSDP.set_state_dict_type(self.task_wrapper, StateDictType.FULL_STATE_DICT, FullStateDictConfig(rank0_only=False), FullOptimStateDictConfig(rank0_only=False))
            self.task_wrapper.load_state_dict(ckpt["task"])
            optim_state_dict = FSDP.optim_state_dict_to_load(self.task_wrapper, self.optimizer, ckpt["optimizer"])
    ...
    def train(self, ):
        ...
        for i in train_iterator:
            ...
            t0 = time.time()  # TODO: Put the timing in callbacks
            train_loss = 0.0
            ...
            if self.gradient_clipping is not False:
                self.task_wrapper.clip_grad_norm_(self.gradient_clipping)
