...
@pe_registry.register("rope_interrope")
class RoPEInterRoPE(nn.Module):
    # rope across chunks
    def __init__(self, dim, max_num_chunk, max_seq_len, chunk_size, base=10000, device=None, **kwargs):
        super().__init__()
        self.rope = RoPE(dim, max_seq_len=max_seq_len, base=base, device=device, **kwargs)
        self.interrope = InterRoPE(dim, max_num_chunk, chunk_size, base, device=device, **kwargs)

    def forward(self, seq_start, seq_end, device, dtype):
        _, rope_emb = self.rope(seq_start, seq_end, device, dtype)
        _, interrope_emb = self.interrope(seq_start, seq_end, device, dtype)
        return None, {"rope": rope_emb["rope"], f"interrope_{self.interrope.chunk_size}": interrope_emb[f"interrope_{self.interrope.chunk_size}"]}

    def step(self, seq_start, seq_end, device, dtype):
        _, rope_emb = self.rope.step(seq_start, seq_end, device, dtype)
        _, interrope_emb = self.interrope.step(seq_start, seq_end, device, dtype)
        return None, {"rope": rope_emb["rope"], f"interrope_{self.interrope.chunk_size}": interrope_emb[f"interrope_{self.interrope.chunk_size}"]}
