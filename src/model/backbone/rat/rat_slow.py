...
@layer_registry.register("rat_slow")
class RATSlow(Base):
    ...
    def forward(self, hidden_states, cache: RATCache=None, **kwargs):
        ...
        x, g, k = map(lambda m: m.reshape(bs, num_chunk, self.chunk_size, self.d_model).transpose(1, 2), (x, g, k))
        g = g.repeat(1, 1, 1, 2) # (b c l d)
        intra_xk = pscan(g, ((1.0 - g) * torch.cat([x, k], dim=-1))).transpose(1, 2).to(torch.bfloat16)
        intra_x = intra_xk[..., :self.d_model].reshape(bs, seq_len, self.num_head, self.d_head).transpose(1, 2)
        intra_k = intra_xk[..., self.d_model:].reshape(bs, seq_len, self.num_head, self.d_head).transpose(1, 2)
